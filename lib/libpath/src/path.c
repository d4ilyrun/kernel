 ...
 */
static const char *find_first_reverse(const char *start, const char *end, bool (*predicate)(char))
{
...
static path_segment_t *path_segment_parse_prev(path_segment_t *prev)
{
	...
	const char *end_prev = find_first_reverse(prev->path->path, prev->start, isnotsep) + 1;
...
bool path_walk_last(const path_t *path, path_segment_t *segment)
{
	...
	const char *end = find_first_reverse(path->path, path->path + path->len, isnotsep) + 1;
