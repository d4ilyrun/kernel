...
#define FOREACH_LLIST_ENTRY(_entry, _list, _field)                              \
	for (_entry = llist_entry(llist_first(_list), typeof(*_entry), _field); \
	     ...
	     _entry = llist_entry(llist_next(&_entry->_field), typeof(*_entry), _field))
...
#define FOREACH_LLIST_ENTRY_SAFE(_entry, _tmp, _list, _field)                       \
	for (_entry = llist_entry(llist_first(_list), typeof(*_entry), _field),     \
	    _tmp = llist_entry(llist_next(&_entry->_field), typeof(*_tmp), _field); \
	     &_entry->_field != llist_head(_list); _entry = _tmp,                   \
	    _tmp = llist_entry(llist_next(&_tmp->_field), typeof(*_entry), _field))
...
#define FOREACH_LLIST_ENTRY_REVERSE(_entry, _list, _field)                     \
	for (_entry = llist_entry(llist_last(_list), typeof(*_entry), _field); \
	     &_entry->_field != llist_head(_list);                             \
	     _entry = llist_entry(llist_prev(&_entry->_field), typeof(*_entry), _field))
...
#define FOREACH_LLIST_ENTRY_REVERSE_SAFE(_entry, _tmp, _list, _field)               \
	for (_entry = llist_entry(llist_last(_list), typeof(*_entry), _field),      \
	    _tmp = llist_entry(llist_prev(&_entry->_field), typeof(*_tmp), _field); \
	     &_entry->_field != llist_head(_list); _entry = _tmp,                   \
	    _tmp = llist_entry(llist_prev(&_tmp->_field), typeof(*_entry), _field))
...
#define llist_first_entry(list, type, member) llist_entry(llist_first(list), type, member)

#define llist_last_entry(list, type, member) llist_entry(llist_last(list), type, member)
 ...
 */
static inline void llist_insert_sorted(llist_t *list, node_t *new, compare_t compare)
{
 ...
 */
static inline bool llist_insert_sorted_unique(llist_t *list, node_t *new, compare_t compare)
{
	node_t *node = llist_first(list);
	int ret;
 ...
 */
static inline node_t *llist_find_first(const llist_t *list, const void *data, compare_t compare)
{
