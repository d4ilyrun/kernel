...
static void avl_equal(avl_t *got, avl_t *expected, const avl_t *parent)
{
	...
	cr_assert_eq(got->parent, parent, "Parent pointer does not point to the actual parent");
...
#define TEST_SEARCH(_n) \
	cr_assert_eq(avl_search(&root->avl, &data##_n.avl, avl_test_compare), &data##_n.avl);
...
Test(Search, DuplicateValues)
{
	...
	cr_assert_eq(avl_search(&root->avl, &data2_bis.avl, avl_test_compare), &data2.avl,
...
Test(Insert, Rotation_Left)
{
	...
	test_data *expected = AVL(root, &data2, &data1, AVL(right, &data4, &data3, &data5));
...
Test(Insert, Rotation_Right)
{
	...
	test_data *expected = AVL(root, &data4, AVL(left, &data2, &data1, &data3), &data5);
...
Test(Insert, Rotation_LeftRight)
{
	...
	test_data *expected = AVL(root, &data7, AVL(left, &data4, &data2, &data6), &data8);
...
Test(Insert, Rotation_RightLeft)
{
	...
	test_data *expected = AVL(root, &data6, &data4, AVL(right, &data8, &data7, &data9));
...
#define TEST_REMOVE(_data, _root, _new_root) TEST_REMOVE1(_data, _root, _new_root, _data);
