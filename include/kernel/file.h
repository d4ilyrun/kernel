...
struct file_operations {
	...
	error_t (*connect)(struct file *, const struct sockaddr *addr, socklen_t len);
...
#define __file_ops(_default, _file, _ops, ...) \
	(_file->ops->_ops ? _file->ops->_ops(_file __VA_OPT__(, ) __VA_ARGS__) : _default)

#define file_ops(_file, _ops, ...) __file_ops(E_NOT_SUPPORTED, _file, _ops, __VA_ARGS__)
