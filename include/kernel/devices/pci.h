...
#define PCI_CLASS(_class, _subclass, _interface) \
	((struct pci_device_class){.class = _class, .subclass = _subclass, .interface = _interface})

/* Value used by device classes that do not use the interface field. */
#define PCI_CLASS_IFACE_EMPTY 0xff

#define PCI_CLASS_NOIFACE(_class, _subclass) PCI_CLASS(_class, _subclass, PCI_CLASS_IFACE_EMPTY)
