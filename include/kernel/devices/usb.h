...
struct usb_device {
	...
	struct usb_hub *hub;
	unsigned int address; /* address of the device on the bus. */
	unsigned int port;
...
static inline const char *udev_name(const struct usb_device *udev)
{
...
struct usb_hub {
	...
	error_t (*get_port_status)(struct usb_hub *, u8 port, u16 *status, u16 *change);
	error_t (*clear_port_feature)(struct usb_hub *, u16 feature, u8 port);
	error_t (*set_port_feature)(struct usb_hub *, u16 feature, u8 port);
};

static inline bool usb_is_root_hub(const struct usb_hub *hub)
{
	return hub->udev->controller->root_hub == hub;
}
 ...
 */
error_t usb_hub_init(struct usb_hub *hub, unsigned int max_ports);
...
#define DECLARE_USB_DRIVER(_name, _driver) DECLARE_DRIVER(_name, _driver, usb_driver_register);
