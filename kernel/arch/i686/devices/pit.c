...
static error_t pit_set_divider(pit_channel channel, u32 value)
{
	...
	case PIT_RW_MSB:
		...
		pit_channel_frequencies[channel] = PIT_INTERNAL_FREQUENCY / (MSB(value) << 8);
		break;
	case PIT_RW:
		outb(counter, LSB(value));
		outb(counter, MSB(value));
		pit_channel_frequencies[channel] = PIT_INTERNAL_FREQUENCY / (value & 0xFFFF);
		break;
	}
	...
	log_dbg("New frequency divisor value for channel %d: %d (%d Hz)", channel, value,
...
{
	if (frequency == 0) {
		log_err("Trying to configure channel %d using NULL frequency. Skip.", channel);
		return E_INVAL;
	}

	if (!IN_RANGE(frequency, PIT_MIN_CHANNEL_FREQUENCY, PIT_MAX_CHANNEL_FREQUENCY)) {
		log_warn("Invalid timer frequency: %dHz (must be between %d and %dHz)", frequency,
			 PIT_MIN_CHANNEL_FREQUENCY, PIT_MAX_CHANNEL_FREQUENCY);
		return E_INVAL;
	}
