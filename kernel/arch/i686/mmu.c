...
#define MMU_RECURSIVE_PAGE_TABLE_ADDRESS(_index) ((page_table_t)FROM_PFN((0xFFC00 + (_index))))
 ...
 */
static void mmu_offset_map(paddr_t start, paddr_t end, int64_t offset, int flags);
...
void mmu_clone(paddr_t destination)
{
	...
	dst_page_directory = vm_alloc_at(&kernel_address_space, destination, PAGE_SIZE,
					 VM_READ | VM_WRITE);
...
bool mmu_map(vaddr_t virtual, paddr_t pageframe, int flags)
{
	...
	if (pte->present) {
		log_err("Allocating already allocated virtual address: " FMT32, virtual);
		return false;
	}
...
static void mmu_init_page_directory(paddr_t page_directory)
{
	...
	kernel_startup_page_directory[MMU_PDE_COUNT - 1].page_table = TO_PFN(page_directory);
	...
	mmu_offset_map(0, KERNEL_HIGHER_HALF_PHYSICAL(KERNEL_CODE_END), KERNEL_HIGHER_HALF_OFFSET,
...
bool mmu_init(void)
{
	...
	interrupts_install_static_handler(PAGE_FAULT, &page_fault_interrupt_handler);
...
page_fault_error:
	...
	PANIC("PAGE FAULT at " FMT32 ": %s access on a %s page %s", faulty_address,
	      error.write ? "write" : "read", error.present ? "protected" : "non-present",
