...
error_t net_interface_add_subnet(struct net_interface *interface, __be ipv4_t addr, uint8_t cidr)
{
...
const struct subnet *net_interface_find_subnet(__be ipv4_t addr)
{
	...
	FOREACH_LLIST (if_node, &registered_net_interfaces) {
		...
		match = llist_find_first(&interface->subnets, (void *)addr, __subnet_match_subnet);
		if (match)
			break;
	}
...
struct net_interface *net_interface_find(__be ipv4_t addr)
{
	...
	FOREACH_LLIST (if_node, &registered_net_interfaces) {
		interface = container_of(if_node, struct net_interface, this);
		match = llist_find_first(&interface->subnets, (void *)addr, __subnet_match_ip);
		if (match)
			break;
	}
