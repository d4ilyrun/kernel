...
static uint16_t ipv4_compute_checksum(struct ipv4_header *iphdr)
{
	...
	iphdr->check = 0;
	iphdr->check = net_internet_checksum((void *)iphdr, ipv4_header_size(iphdr));
...
/*
 * Common codepath when connecting an AF_INET socket to a peer.
 */
...
static error_t af_inet_verify_addr(const struct sockaddr *addr, socklen_t len)
{
...
static struct socket_domain af_inet = {
    .domain = AF_INET,
    .socket_init = af_inet_socket_init,
    .verify_addr = af_inet_verify_addr,
};

static error_t ipv4_init(void)
{
