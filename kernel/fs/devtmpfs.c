 ...
 */
static vnode_t *
devtmpfs_root_vnode_create(vnode_t *node, const char *name, vnode_type type, mode_t mode)
{
	UNUSED(node);
	UNUSED(name);
	UNUSED(type);
 ...
 *
 */
static error_t devtmpfs_root_vnode_getdents(vnode_t *vnode, off_t *offp, void *buf, size_t *sizep)
{
 ...
 *
 */
static vnode_t *devtmpfs_root_vnode_lookup(vnode_t *node, const path_segment_t *child)
{
	device_t *dev;
