 ...
 */
static ssize_t tar_read(struct tar_filesystem *fs, void *buffer, off_t offset, size_t size)
{
...
static void tar_node_set_name(struct tar_node *node, const char *name, size_t size)
{
...
static struct tar_node *tar_node_alloc(struct tar_filesystem *fs)
{
	...
	if (new == NULL) {
		log_err("%s: failed to allocate memory for a new node", blkdev->dev.name);
 ...
 */
static struct tar_node *tar_node_from_header(struct tar_filesystem *fs, off_t hdr_offset)
{
	struct tar_node *new;
 ...
 */
static struct tar_node *tar_virtual_dir_node(struct tar_filesystem *fs, path_segment_t *segment)
{
	struct tar_node *new;
...
static vnode_t *tar_vnode_lookup(vnode_t *node, const path_segment_t *child)
{
	...
	tree_node_t *tar_child = tree_find_child(&tar_node->this, tar_node_is, child);
...
static vnode_t *tar_vnode_create(vnode_t *vnode, const char *name, vnode_type type, mode_t mode)
{
	UNUSED(vnode);
	UNUSED(name);
	UNUSED(type);
 ...
 *
 */
static error_t tar_vnode_getdents(vnode_t *vnode, off_t *offp, void *buf, size_t *sizep)
{
