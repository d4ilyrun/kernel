...
#define INITCALL_SECTION(_level)                                        \
	...
	for (struct initcall *initcall = section->start; initcall < section->end; initcall += 1) {
		log(LOG_LEVEL_DEBUG, "initcall", "%s", initcall->name);
		err = initcall->call();
		if (err)
			log(LOG_LEVEL_WARN, "initcall", "%s failed with %pe", initcall->name, &err);
	}
}
...
static error_t kernel_mount_initfs(struct multiboot_tag_module *module)
{
	...
	log_info("found initrd @ [" FMT32 ":" FMT32 "]", module->mod_start, module->mod_end);
