...
void kernel_task_elf(MAYBE_UNUSED void *data)
{
    ...
    if (IS_ERR(busybox)) {
        log_err("failed to open 'busybox': %s",
                err_to_str(ERR_FROM_PTR(busybox)));
        return;
    }

    in_memory = mmap_file(NULL, file_size(busybox), PROT_READ, MAP_KERNEL,
                          busybox);
    if (in_memory == MMAP_INVALID) {
        log_err("failed to read 'busybox'");
        return;
    }

    err = execfmt_execute(in_memory);
    log_info("busybox execution: %s", err_to_str(err));

    munmap(in_memory, file_size(busybox));
}
