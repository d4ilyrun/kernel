...
uint32_t pci_read_config(uint8_t bus, uint8_t device, uint8_t offset, size_t size)
{
...
void pci_write_config(uint8_t bus, uint8_t device, uint8_t offset, size_t size, uint32_t value)
{
	uint32_t size_mask = BIT64(size * 8) - 1;
	unsigned int reg_offset = offset % sizeof(uint32_t);
	uint32_t cfg_address;
