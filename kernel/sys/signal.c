...
static const enum signal_action_type signal_actions_default[SIGNAL_COUNT] = {
    [SIGABRT] = SIG_ACTION_ABORT,	[SIGALRM] = SIG_ACTION_TERMINATE,
    [SIGBUS] = SIG_ACTION_ABORT,	[SIGCHLD] = SIG_ACTION_IGNORE,
    [SIGCONT] = SIG_ACTION_CONTINUE,	[SIGFPE] = SIG_ACTION_ABORT,
    [SIGHUP] = SIG_ACTION_TERMINATE,	[SIGILL] = SIG_ACTION_ABORT,
    [SIGINT] = SIG_ACTION_TERMINATE,	[SIGKILL] = SIG_ACTION_TERMINATE,
    [SIGPIPE] = SIG_ACTION_TERMINATE,	[SIGQUIT] = SIG_ACTION_ABORT,
    [SIGSEGV] = SIG_ACTION_ABORT,	[SIGSTOP] = SIG_ACTION_STOP,
    [SIGTERM] = SIG_ACTION_TERMINATE,	[SIGTSTP] = SIG_ACTION_STOP,
    [SIGTTIN] = SIG_ACTION_STOP,	[SIGTTOU] = SIG_ACTION_STOP,
    [SIGUSR1] = SIG_ACTION_TERMINATE,	[SIGUSR2] = SIG_ACTION_TERMINATE,
    [SIGWINCH] = SIG_ACTION_IGNORE,	[SIGSYS] = SIG_ACTION_ABORT,
    [SIGTRAP] = SIG_ACTION_ABORT,	[SIGURG] = SIG_ACTION_IGNORE,
    [SIGVTALRM] = SIG_ACTION_TERMINATE, [SIGXCPU] = SIG_ACTION_ABORT,
 ...
 *
 */
static void signal_queue_push(struct signal_queue *queue, struct signal_context *sig_ctx)
{
 ...
 */
static size_t signal_queue_flush_signals(struct signal_queue *queue, sigset_t to_remove)
{
 ...
 */
static void signal_deliver_terminate(struct thread *thread,
 ...
 *
 */
static void signal_deliver_abort(struct thread *thread,
				 const struct signal_action *sig_action MAYBE_UNUSED,
				 const struct signal_context *sig_ctx)
{
	u16 status;
 ...
 */
static void signal_deliver_ignore(struct thread *thread, const struct signal_action *sig_action,
				  const struct signal_context *sig_ctx)
{
	UNUSED(thread);
	UNUSED(sig_action);
 ...
 * TODO: SIGSTOP
 */
static void signal_deliver_stop(struct thread *thread, const struct signal_action *sig_action,
				const struct signal_context *sig_ctx)
{
 ...
 * TODO: SIGCONT
 */
static void signal_deliver_continue(struct thread *thread, const struct signal_action *sig_action,
				    const struct signal_context *sig_ctx)
{
 ...
 */
static void signal_deliver_catch(struct thread *thread, const struct signal_action *sig_action,
				 const struct signal_context *sig_ctx)
{
...
typedef void (*signal_deliver_func_t)(struct thread *, const struct signal_action *,
				      const struct signal_context *);

static const signal_deliver_func_t signal_deliver_functions[] = {
    [SIG_ACTION_TERMINATE] = signal_deliver_terminate, [SIG_ACTION_ABORT] = signal_deliver_abort,
    [SIG_ACTION_IGNORE] = signal_deliver_ignore,       [SIG_ACTION_STOP] = signal_deliver_stop,
    [SIG_ACTION_CONTINUE] = signal_deliver_continue,   [SIG_ACTION_CATCH] = signal_deliver_catch,
};
 ...
 */
static inline bool signal_can_generate(struct thread *sender, struct thread *receiver)
{
 ...
 */
error_t signal_generate(struct thread *thread, const siginfo_t *sig_info, bool to_thread)
{
	struct signal_context *sig_ctx;
				       ...
				       struct signal_action *old_sig_action)
{
	 ...
	 */
	if (signo == SIGCHLD) {
		if (sa_action->sa_flags & SA_NOCLDWAIT || sa_action->sa_handler == SIG_IGN ||
...
int sys_sigaction(int sig, const struct sigaction *act, struct sigaction *oact)
{
	...
	err = signal_action_configure(current->process->sig_set, sig, &sig_action, &old_sig_action);
	if (err)
		return err;
...
{
	kmem_cache_sigset = kmem_cache_create("signal_set", sizeof(struct signal_set), 1,
					      signal_set_constructor, NULL);
	kmem_cache_sigctx = kmem_cache_create("signal", sizeof(struct signal_context), 1, NULL,
					      NULL);
