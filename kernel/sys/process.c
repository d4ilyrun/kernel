 ...
 */
extern error_t arch_thread_init(thread_t *, thread_entry_t, void *data, void *esp, void *ebp);
...
void process_init_kernel_process(void)
{
	...
	llist_add(&kernel_process.threads, &kernel_process_initial_thread.this_proc);
	...
	if (IS_ERR(kernel_process.as))
		PANIC("Failed to create kernel user address space: %pE", kernel_process.as);
	...
	ustack = vm_alloc(kernel_process.as, USER_STACK_SIZE, VM_READ | VM_WRITE | VM_CLEAR);
	if (ustack == NULL)
...
thread_t *thread_spawn(struct process *process, thread_entry_t entrypoint, void *data, void *esp,
		       void *ebp, u32 flags)
{
	thread_t *thread;
	void *kstack = NULL;
	...
	kstack = vm_alloc(&kernel_address_space, KERNEL_STACK_SIZE, VM_KERNEL_RW | VM_CLEAR);
	if (kstack == NULL) {
...
struct thread *process_execute_in_userland(const char *exec_path)
{
	...
	thread = thread_fork(current, __process_execute_in_userland, (void *)exec_path);
	if (IS_ERR(thread))
		return thread;
...
struct thread *thread_fork(struct thread *thread, thread_entry_t entrypoint, void *arg)
{
	...
	if (IS_KERNEL_ADDRESS(entrypoint) && entrypoint != __process_execute_in_userland)
...
pid_t sys_fork(void)
{
	...
	fork = thread_fork(current, thread_get_interrupt_return_address(current), NULL);
	if (IS_ERR(fork))
...
void thread_deliver_pending_signal(struct thread *thread)
{
	 ...
	 */
	sig_ctx = signal_queue_pop(&thread->sig_pending, thread->sig_blocked);
	if (!sig_ctx)
		sig_ctx = signal_queue_pop(&thread->process->sig_pending, thread->sig_blocked);
