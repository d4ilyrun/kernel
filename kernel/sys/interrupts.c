 ...
 *
 */
static error_t interrupt_chip_interrupt_handle(const struct interrupt_chip *chip, unsigned int nr)
{
 ...
 *
 */
static error_t interrupt_chip_install_handler(struct interrupt_chip *chip, unsigned int nr,
					      struct interrupt_handler *handler)
{
...
interrupt_chip_get_handler(const struct interrupt_chip *chip, unsigned int nr, void **pdata)
{
 ...
 *
 */
const char *interrupt_chip_interrupt_name(const struct interrupt_chip *chip, unsigned int nr)
{
	if (nr >= chip->interrupt_count)
		return "invalid";
 ...
 *
 */
error_t interrupts_install_static_handler(unsigned int nr, struct interrupt_handler *handler)
{
					    ...
					    interrupt_handler_func_t threaded, void *data)
{
	...
	if (threaded) {
		thread = thread_spawn(&kernel_process, threaded_interrupt, interrupt_handler, NULL,
				      NULL, THREAD_KERNEL);
		if (thread == NULL) {
