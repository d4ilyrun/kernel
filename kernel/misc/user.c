 ...
 *
 */
static error_t creds_set_euid(struct user_creds *dst, uid_t euid)
{
    if (!uid_is_valid(euid))
        return E_INVAL;

    if (!creds_has_uid_privileges(dst) &&
        euid != dst->ruid &&
        euid != dst->euid &&
        euid != dst->suid)
        return E_PERM;
 ...
 *
 */
static error_t creds_set_egid(struct user_creds *dst, gid_t egid)
{
...
error_t sys_setuid(uid_t uid)
{
    ...
    locked_scope (&process->lock) {
        ...
        if (!creds_has_uid_privileges(creds)) {
            ret = creds_set_euid(&process->creds, uid);
        } else {
            creds->ruid = uid;
            creds->euid = uid;
            creds->suid = uid;
 ...
 *
 */
error_t sys_seteuid(uid_t euid)
{
    struct process *process = current->process;
    error_t ret = E_SUCCESS;

    locked_scope (&process->lock) {
        ret = creds_set_euid(&process->creds, euid);
    }
 ...
 *
 */
error_t sys_setreuid(uid_t ruid, uid_t euid)
{
    struct process *process = current->process;
    struct user_creds creds;
    error_t ret;
 ...
 *
 */
error_t sys_setresuid(uid_t ruid, uid_t euid, uid_t suid)
{
    struct process *process = current->process;
 ...
 *
 */
error_t sys_setegid(gid_t egid)
{
    struct process *process = current->process;
    error_t ret = E_SUCCESS;
 ...
 *
 */
error_t sys_setregid(gid_t rgid, gid_t egid)
{
    struct process *process = current->process;
    struct user_creds creds;
    error_t ret;
 ...
 *
 */
error_t sys_setresgid(gid_t rgid, gid_t egid, gid_t sgid)
{
    struct process *process = current->process;
    struct user_creds *old_creds;
    struct user_creds new_creds;
