 ...
 *
 */
static int creds_set_euid(struct user_creds *dst, const struct user_creds *cur, uid_t euid)
{
	if (!uid_is_valid(euid))
		return -E_INVAL;

	if (!creds_has_uid_privileges(cur) && euid != cur->ruid && euid != cur->euid &&
	    euid != cur->suid)
		return -E_PERM;
...
static int creds_set_egid(struct user_creds *dst, gid_t egid)
{
	...
	if (!creds_has_gid_privileges(dst) && egid != dst->rgid && egid != dst->egid &&
	    egid != dst->sgid)
		return -E_PERM;
...
int sys_setreuid(uid_t ruid, uid_t euid)
{
	...
	if (ruid != UID_NOT_SET) {
		if (!creds_has_uid_privileges(new_creds) && ruid != new_creds->ruid &&
		    ruid != new_creds->euid) {
			ret = -E_PERM;
			goto out;
		}
	...
	if ((ruid != UID_NOT_SET) || (euid != UID_NOT_SET && euid != old_creds->ruid))
...
#define __setresuid(_uid)                                                                      \
	do {                                                                                   \
		...
		ret = -E_PERM;                                                                 \
		if (_uid != UID_NOT_SET) {                                                     \
			if (!creds_has_uid_privileges(old_creds) && _uid != old_creds->ruid && \
			    _uid != old_creds->suid && _uid != old_creds->euid)                \
				goto out;                                                      \
			new_creds->_uid = _uid;                                                \
		}                                                                              \
	} while (0)
...
int sys_setregid(gid_t rgid, gid_t egid)
{
	...
	ret = -E_PERM;
	if (rgid != GID_NOT_SET) {
		if (!creds_has_gid_privileges(old_creds) && rgid != old_creds->rgid &&
		    rgid != old_creds->egid)
			goto out;
		new_creds->rgid = rgid;
	}
	...
	if ((rgid != GID_NOT_SET) || (egid != GID_NOT_SET && egid != old_creds->rgid))
...
#define __setresgid(_gid)                                                                      \
	do {                                                                                   \
		...
		ret = -E_PERM;                                                                 \
		if (_gid != GID_NOT_SET) {                                                     \
			if (!creds_has_gid_privileges(old_creds) && _gid != old_creds->rgid && \
			    _gid != old_creds->sgid && _gid != old_creds->egid)                \
				goto out;                                                      \
			new_creds->_gid = _gid;                                                \
		}                                                                              \
	} while (0)
...
{
	user_creds_cache = kmem_cache_create("user_creds", sizeof(struct user_creds),
					     CPU_CACHE_ALIGN, creds_constructor, NULL);
