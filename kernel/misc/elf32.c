...
static error_t elf32_load(struct executable *executable, void *data)
{
    ...
    ctx = kcalloc(1, sizeof(*ctx), KMALLOC_KERNEL);
    if (ctx == NULL)
        return E_NOMEM;

    executable->data = ctx;
    ctx->entrypoint = (void *)elf->e_entry;
...
static error_t elf32_execute(struct executable *executable)
{
    const struct elf32_ctx *ctx = executable->data;
    log_dbg("executing ELF binary @ %p", ctx->entrypoint);
    ctx->entrypoint();
    return E_SUCCESS;
}
