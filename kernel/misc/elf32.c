...
static inline void *elf32_at_offset(const struct elf32_ehdr *elf, elf32_off offset)
{
	return (void *)elf + offset;
}

static inline const struct elf32_phdr *elf32_program_header(const struct elf32_ehdr *elf)
{
	return elf32_at_offset(elf, elf->e_phoff);
}

static inline const struct elf32_shdr *elf32_section_table(const struct elf32_ehdr *elf)
{
	return elf32_at_offset(elf, elf->e_shoff);
}

static const struct elf32_shdr *elf32_section(const struct elf32_ehdr *elf, size_t index)
{
...
elf32_section_name(const struct elf32_ehdr *elf, const struct elf32_shdr *section)
{
...
static inline const char *elf32_symbol_name(const struct elf32_ehdr *elf,
...
static bool is_elf32(const void *data)
{
	...
	return hdr->e_ident[EI_MAG0] == EI_MAG_0 && hdr->e_ident[EI_MAG1] == EI_MAG_1 &&
	       hdr->e_ident[EI_MAG2] == EI_MAG_2 && hdr->e_ident[EI_MAG3] == EI_MAG_3;
}
...
static error_t elf32_load_segment(const struct elf32_ehdr *elf, const struct elf32_phdr *segment)
{
	void *in_memory;
	void *allocated;
	void *in_file;
	...
	if (segment->p_memsz < segment->p_filesz) {
		log_info("size in memory inferior to size in file (%d < %d)", segment->p_memsz,
			 segment->p_filesz);
		return E_INVAL;
	}
	 ...
	 */
	allocated = vm_alloc_start(current->process->as, in_memory, size, VM_KERNEL_RW | VM_FIXED);
	if (!allocated) {
 ...
 */
static error_t
elf32_apply_segment_protection(const struct elf32_ehdr *elf, const struct elf32_phdr *segment)
{
	vm_flags_t prot = 0;
	error_t err;
	void *start;
...
static error_t elf32_load_symbol(const struct elf32_ehdr *elf, const struct elf32_sym *symbol)
{
	UNUSED(elf);
	UNUSED(symbol);
...
static error_t
elf32_load_symbol_section(const struct elf32_ehdr *elf, const struct elf32_shdr *section)
{
...
static error_t elf32_load_section(const struct elf32_ehdr *elf, const struct elf32_shdr *section)
{
	...
	case SHT_PROGBITS:
		memcpy(start, elf32_at_offset(elf, section->sh_offset), section->sh_size);
		break;
	case SHT_NOBITS:
