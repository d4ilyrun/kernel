...
void *uacpi_kernel_map(uacpi_phys_addr physical, uacpi_size len)
{
	...
	page = vm_alloc_at(&kernel_address_space, physical, len, VM_READ | VM_WRITE);
	if (IS_ERR(page))
		return UACPI_NULL;
...
uacpi_status
uacpi_kernel_io_read(uacpi_handle base, uacpi_size offset, uacpi_u8 byte_width, uacpi_u64 *value)
{
	// mapped 1:1 by uacpi_kernel_io_map, so we can read as with a raw address
	return uacpi_kernel_raw_io_read((uacpi_io_addr)(native_t)base + offset, byte_width, value);
}

uacpi_status
uacpi_kernel_io_write(uacpi_handle base, uacpi_size offset, uacpi_u8 byte_width, uacpi_u64 value)
{
	// mapped 1:1 by uacpi_kernel_io_map, so we can write as with a raw address
	return uacpi_kernel_raw_io_write((uacpi_io_addr)(native_t)base + offset, byte_width, value);
}
...
uacpi_kernel_schedule_work(uacpi_work_type type, uacpi_work_handler handler, uacpi_handle ctx)
{
	...
	thread_t *thread = thread_spawn(&kernel_process, handler, ctx, NULL, NULL, THREAD_KERNEL);
	if (thread == NULL)
		return UACPI_STATUS_OUT_OF_MEMORY;
...
void uacpi_kernel_vlog(uacpi_log_level level, const uacpi_char *format, uacpi_va_list va_args)
{
