...
execfmt_execute_executable(struct executable *executable, void *stack_pointer, void *base_pointer)
{
	...
	thread_jump_to_userland(stack_pointer, base_pointer, executable->entrypoint, NULL);
}
 ...
 */
static void *
execfmt_push_string(const char *string, void *stack_top, void *stack_bottom, size_t *pushed)
{
	size_t len;
 ...
 *
 */
static void *execfmt_push_strings(char *const strings[], void *stack_top, void *stack_bottom,
 ...
 */
static void *execfmt_push_params(struct exec_params *params, void *stack_top, void *stack_bottom,
				 size_t *argv_size, size_t *envp_size)
{
	size_t pushed = 0;

	stack_top = execfmt_push_strings(params->envp, stack_top, stack_bottom, params->envpc,
					 &pushed);
	if (IS_ERR(stack_top))
		return stack_top;
	...
	*envp_size = pushed;
	...
	stack_top = execfmt_push_strings(params->argv, stack_top, stack_bottom, params->argc,
					 &pushed);
	if (IS_ERR(stack_top))
		return stack_top;
 ...
 */
static void *execfmt_pop_strings(char *strings, void *stack_top, void *stack_bottom, size_t size)
{
 ...
 */
static void *execfmt_pop_params(const struct exec_params *params, void *stack_top,
				void *stack_bottom, size_t argv_size, size_t envp_size)
{
	void *envp;
	void *argv;

	stack_top = execfmt_pop_strings((char *)params->envp, stack_top, stack_bottom, envp_size);
	if (IS_ERR(stack_top))
		return stack_top;
	envp = stack_top;

	stack_top = execfmt_pop_strings((char *)params->argv, stack_top, stack_bottom, argv_size);
	if (IS_ERR(stack_top))
		return stack_top;
	argv = stack_top;
...
error_t execfmt_execute(struct exec_params *params)
{
	...
	args_buffer = vm_alloc(&kernel_address_space, EXECFMT_ARGS_BUFFER_SIZE, VM_KERNEL_RW);
	if (!args_buffer) {
	...
	args_buffer_bottom = execfmt_push_params(params, args_buffer_bottom, args_buffer,
						 &argv_size, &envp_size);
	...
	process_set_name(current->process, segment.start, path_segment_length(&segment));
	...
	ustack = vm_alloc(current->process->as, USER_STACK_SIZE, VM_USER_RW | VM_CLEAR);
	...
	ustack = execfmt_pop_params(params, thread_get_user_stack_top(current),
				    thread_get_user_stack(current), argv_size, envp_size);
	if (IS_ERR(ustack)) {
