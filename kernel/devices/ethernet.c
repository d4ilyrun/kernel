...
struct ethernet_device *ethernet_device_alloc(size_t priv_size)
{
	...
	total_size = priv_size +
		     align_up(sizeof(struct ethernet_device), ETHERNET_DEVICE_PRIV_ALIGNMENT);
...
error_t ethernet_device_register(struct ethernet_device *device)
{
	...
	if (ret) {
		log_err("%s: failed to initialize worker thread: %pe", ethernet_device_name(device),
			&ret);
...
ethernet_device_find_by(int (*match)(const void *dev_node, const void *data), const void *data)
{
	node_t *dev_node;
...
void ethernet_device_receive_packet(struct ethernet_device *netdev, struct packet *packet)
{
	no_preemption_scope () {
		queue_enqueue(&netdev->rx_queue, &packet->rx_this);
		if (!worker_running(netdev->worker))
			worker_start(netdev->worker, __ethernet_device_receive_packet, netdev);
	}
}
