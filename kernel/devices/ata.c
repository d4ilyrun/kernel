...
static inline void ata_bus_read_pio_sector(const struct ata_bus *bus, void *buffer)
{
	insw(bus->io + ATA_DATA, buffer, ATA_SECTOR_SIZE);
}

static inline void ata_bus_write_pio_sector(const struct ata_bus *bus, const void *buffer)
{
...
static void ata_bus_request_read_sectors(struct ata_bus *bus, struct block_io_request *request)
{
...
static void ata_bus_request_write_sectors(struct ata_bus *bus, struct block_io_request *request)
{
	for (blkcnt_t count = 0; count < request->count; ++count) {
...
static error_t ata_request(struct block_device *blkdev, struct block_io_request *request)
{
...
static struct ata_drive *ata_drive_identify(struct ata_bus *bus)
{
	...
	default:
		log_warn("unsupported ATA signature: " FMT8 ":" FMT8, lba_mid, (uint8_t)lba_high);
		err = E_NOT_SUPPORTED;
		goto identify_error_exit;
	}
