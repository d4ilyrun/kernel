...
#define ATA_IDENTIFY_SECTORS_LBA28 60
...
static void ata_bus_request_read_sectors_sync(struct ata_bus *bus,
                                              struct block_io_request *request)
{
    for (blkcnt_t count = 0; count < request->count; ++count) {
        ata_bus_command(bus, ATA_COMMAND_READ_SECTORS);
        waitqueue_enqueue(&bus->waitqueue, current);
...
static void ata_bus_request_write_sectors_sync(void *data)
{
    struct ata_bus *bus = data;
    struct block_io_request *request = bus->request;
...
{
    bus->request = request;
    worker_start(&bus->worker, ata_bus_request_write_sectors_sync, bus);
}
...
ata_request(struct block_device *blkdev, struct block_io_request *request)
{
    ...
    /*
     * The lock should be released once the last request has been finished.
     */
    spinlock_acquire(&bus->lock);

    ata_drive_select(drive);
    ata_bus_wait_ready(bus);

    ata_bus_set_lba(bus, request->offset);
    outb(bus->io + ATA_SECTOR_COUNT, request->count);

    switch (request->type) {
    case BLOCK_IO_REQUEST_READ:
        ata_bus_request_read_sectors_sync(bus, request);
        break;
...
static struct ata_drive *ata_drive_identify(struct ata_bus *bus)
{
    ...
    ata_bus_control_set(bus, ATA_DEVICE_CONTROL_NIEN); /* disable interrupts */
    ...
    ata_bus_read_pio_sector(bus, &sector);

    sector_count = *(uint32_t *)&sector[ATA_IDENTIFY_SECTORS_LBA28];
    if (sector_count == 0) {
    ...
    ata_bus_control_mask(bus, ATA_DEVICE_CONTROL_NIEN);
...
static error_t ata_interrupt_handler(void *data)
{
    struct ata_bus *bus = data;
    uint8_t status;

    /*
     * Interrput was not for us (shared).
     */
    status = ata_bus_status(bus);
    if (status & ATA_STATUS_BSY)
        return E_SUCCESS;

    waitqueue_dequeue_all(&bus->waitqueue);
    pic_eoi(bus->irq);
...
static error_t ata_bus_probe(uint16_t io, uint16_t control, int irq)
{
    ...
    INIT_WAITQUEUE(bus->waitqueue);

    err = worker_init(&bus->worker);
    if (err) {
        log_warn("failed to initialize worker: %s", err_to_str(err));
        goto bus_probe_err;
    }
    ...
    interrupts_set_handler(PIC_MASTER_VECTOR + irq, ata_interrupt_handler, bus);
