...
static inline void ramdisk_read(struct ramdisk *ramdisk, char *buffer, off_t off, size_t size)
{
	memcpy(buffer, (void *)ramdisk->segment->start + off, size);
}

static inline void
ramdisk_write(struct ramdisk *ramdisk, const char *buffer, off_t off, size_t size)
{
...
static error_t ramdisk_block_request(struct block_device *blkdev, struct block_io_request *request)
{
	struct ramdisk *ramdisk = to_ramdisk(blkdev);

	switch (request->type) {
	case BLOCK_IO_REQUEST_READ:
		ramdisk_read(ramdisk, request->buf, request->offset, request->count * PAGE_SIZE);
		break;
	case BLOCK_IO_REQUEST_WRITE:
		ramdisk_write(ramdisk, request->buf, request->offset, request->count * PAGE_SIZE);
		break;
	}
