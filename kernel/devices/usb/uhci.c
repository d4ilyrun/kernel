...
static void uhci_td_dump(const struct uhci_td *td)
{
	...
	log_info("token:  %s%s (address=%u endpoint=%#02x length=%u)",
		 ...
		 (token & UHCI_TD_TOKEN_EP_MASK) >> UHCI_TD_TOKEN_EP_OFFSET, maxlen);

	log_info("status: %s%s%s%s", (status & UHCI_TD_STATUS_ACTIVE) ? "ACTIVE " : "",
 ...
 *
 */
static inline void uhci_td_fill(const struct usb_device *udev, struct uhci_td *td, enum usb_pid pid,
				struct usb_pipe *pipe, paddr_t dma_addr, size_t maxlen)
{
	...
	td->token |= (pipe->endpoint << UHCI_TD_TOKEN_EP_OFFSET) & UHCI_TD_TOKEN_EP_MASK;
	td->token |= (udev->address << UHCI_TD_TOKEN_ADDR_OFFSET) & UHCI_TD_TOKEN_ADDR_MASK;
	td->token |= ((maxlen - 1) << UHCI_TD_TOKEN_MAXLEN_OFFSET) & UHCI_TD_TOKEN_MAXLEN_MASK;
}
 ...
 */
static error_t
uhci_urb_submit_control(struct uhci_controller *uhci, struct usb_device *udev, struct urb *urb)
{
 ...
 */
static inline bool uhci_td_is_dummy(const struct uhci_qh *qh, const struct uhci_td *td)
{
 ...
 */
static void uhci_process_interrupt_urbs(struct uhci_controller *uhci, enum uhci_qh_prio qh_idx)
{
	struct uhci_qh *qh = uhci->qhs[qh_idx];
	struct uhci_td *td;
	paddr_t paddr;
 ...
 */
static interrupt_return_t uhci_interrupt(void *data)
{
	struct uhci_controller *uhci = data;
	u32 status;
...
{
	struct pci_device *pdev = uhci->controller->pcidev;
	error_t err;

	err = pci_device_install_threaded_interrupt_handler(pdev, NULL, uhci_interrupt, uhci);
	if (err) {
		log_err("failed to register interrupt: %pe", &err);
		return err;
	}

	uhci_outw(uhci, UHCI_USBINTR, UHCI_USBINTR_IOC | UHCI_USBINTR_RESUME);
...
static error_t uhci_init_framelist(struct uhci_controller *uhci)
{
	...
	uhci->frame_list = kmalloc_dma(uhci->frame_list_size * sizeof(*uhci->frame_list));
	if (!uhci->frame_list)
		return E_NOMEM;
 ...
 *
 * NOTE: When disabled the controller first completes the current transaction
 *       before halting.
 */
static inline void uhci_enable_transfer(struct uhci_controller *uhci, bool enable)
{
	u16 val;
 ...
 *
 */
static error_t uhci_hub_set_port_feature(struct usb_hub *hub, u16 feature, u8 port)
{
	struct uhci_controller *uhci = hub->udev->controller->priv;
	u16 mask;
	u16 val;

	if (feature == USB_HUB_FEAT_PORT_POWER)
		return E_SUCCESS;
 ...
 *
 */
static error_t uhci_hub_clear_port_feature(struct usb_hub *hub, u16 feature, u8 port)
{
	struct uhci_controller *uhci = hub->udev->controller->priv;
	u16 mask;
	u16 val;
 ...
 *
 */
static error_t uhci_hub_get_port_status(struct usb_hub *hub, u8 port, u16 *status, u16 *change)
{
	struct uhci_controller *uhci = hub->udev->controller->priv;
	u16 wPortStatus = 0;
	u16 wPortChange = 0;
	...
	if (val & UHCI_PORTSC_CCS)
		wPortStatus |= USB_HUB_STATUS_PORT_CONNECTION;
	if (val & UHCI_PORTSC_PED)
		wPortStatus |= USB_HUB_STATUS_PORT_ENABLE;
	if (val & UHCI_PORTSC_SUSPEND)
		wPortStatus |= USB_HUB_STATUS_PORT_SUSPEND;
	if (val & UHCI_PORTSC_RESET)
		wPortStatus |= USB_HUB_STATUS_PORT_RESET;
	if (val & UHCI_PORTSC_LSDA)
		wPortStatus |= USB_HUB_STATUS_PORT_LOW_SPEED;

	if (val & UHCI_PORTSC_CSC)
		wPortChange |= USB_HUB_STATUS_C_PORT_CONNECTION;
	if (val & UHCI_PORTSC_PEDC)
		wPortChange |= USB_HUB_STATUS_C_PORT_ENABLE;
...
static error_t uhci_init_root_hub(struct usb_controller *controller)
{
	...
	device_set_name(&udev->dev, "uhci-root-hub");
	...
	err = usb_hub_init(hub, UHCI_HUB_PORT_COUNT);
	if (err) {
		usb_device_destroy(udev);
		return err;
	}
...
static error_t uhci_controller_init(struct usb_controller *controller)
{
	...
	uhci->io_reg = pci_device_read_config(pcidev, UHCI_PCI_USBBASE, UHCI_PCI_USBBASE_SIZE);
...
{
	kmem_cache_uhci_td = kmem_cache_create("uhci:td", sizeof(struct uhci_td), CPU_CACHE_ALIGN,
					       NULL, NULL);
	if (!kmem_cache_uhci_td)
		return E_NOMEM;
