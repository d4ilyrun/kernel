...
static error_t usb_set_port_feature(struct usb_hub *hub, u16 feature, u16 port)
{
	...
	return usb_device_request_raw(hub->udev,
				      USB_SETUP_REQ_TYPE_CLASS | USB_SETUP_REQ_TYPE_FOR_PORT,
				      USB_REQUEST_SET_FEATURE, feature, port, NULL, 0);
}

static error_t usb_clear_port_feature(struct usb_hub *hub, u16 feature, u16 port)
{
	if (!is_usb_port_feature(feature))
		return E_INVAL;
	if (port <= 0 || port > hub->max_port)
	...
	return usb_device_request_raw(hub->udev,
				      USB_SETUP_REQ_TYPE_CLASS | USB_SETUP_REQ_TYPE_FOR_PORT,
				      USB_REQUEST_CLEAR_FEATURE, feature, port, NULL, 0);
}

static error_t usb_get_port_status(struct usb_hub *hub, u16 port, u16 *status, u16 *change)
{
	le32_t raw_status;
	error_t err;
	...
	err = usb_device_request_raw(hub->udev,
				     USB_SETUP_REQ_TYPE_CLASS | USB_SETUP_REQ_TYPE_FOR_PORT,
				     USB_REQUEST_GET_STATUS, 0, port, &raw_status, 4);
	if (err)
		return err;
...
/*
 * @see USB 2.0 - 9.1.2 (Bus Enumeration) & 11.5 (HUB - Downstream facing ports).
 */
static error_t
usb_hub_init_port_device(struct usb_hub *hub, struct usb_device *udev, unsigned int port)
{
	static DECLARE_MUTEX(default_address_lock);
	bool use_default_address = false;
	...
	/* Disconnected state. */
	err = usb_set_port_feature(hub, USB_HUB_FEAT_PORT_POWER, port);
	if (err) {
		log_err("%s: failed to power port %d (%pe)", udev_name(hub->udev), port, &err);
		goto fail;
	}
	...
	if (err) {
		log_err("%s: failed to reset port %d (%pe)", udev_name(hub->udev), port, &err);
		goto fail;
	}

	timer_delay_ms(30);

	usb_get_port_status(hub, port, &status, NULL);
	if (!(status & USB_HUB_STATUS_PORT_ENABLE)) {
		/*
		 * The hub should normally put the downstream port into the Enabled state
		 * after the resetting period, but dome hubs don't (e.g. qemu's UHCI root hub).
		 */
		err = usb_set_port_feature(hub, USB_HUB_FEAT_PORT_ENABLE, port);
		if (err) {
			log_err("%s: failed to enable port %d (%pe)", udev_name(hub->udev), port, &err);
			goto fail;
		}
	}

	/* At this stage the HUB's port has been properly enabled, and the device
	 * responds to the default USB address.
	 *
	 * Assign a proper USB address to the device (switch to Address state).
	 */
	err = usb_device_request(udev, USB_REQUEST_SET_ADDRESS, addr, 0, NULL, 0);
	if (err) {
		log_err("%s: failed to set address %d (%pe)", udev_name(hub->udev), port, &err);
		goto fail;
	}
	udev->address = addr;
...
fail:
	usb_clear_port_feature(hub, USB_HUB_FEAT_PORT_POWER, port);
 ...
 *
 */
static error_t usb_hub_connect_device(struct usb_hub *hub, unsigned int port, enum usb_speed speed)
{
	struct usb_device *udev;
	error_t err;

	log_info("%s: new device connected on port %d", udev_name(hub->udev), port);
	...
	/* Device has been enabled and is in the Address state.
	 * Let the device's driver handle the configuration. */
	err = usb_device_probe(udev);
	if (err) {
		log_warn("failed to probe USB device: %pe", &err);
		goto fail;
	}
	hub->ports[port] = udev;
...
static void usb_hub_disconnect_device(struct usb_hub *hub, unsigned int port)
{
	...
	log_info("%s: device disconnected (port %d)", udev_name(hub->udev), port);
 ...
 *
 */
error_t usb_hub_init(struct usb_hub *hub, unsigned int max_port)
{
	struct usb_device *udev = hub->udev;
...
static error_t usb_hub_probe(struct device *device)
{
	...
	err = usb_device_request_raw(udev, USB_SETUP_REQ_TYPE_CLASS, USB_REQUEST_GET_DESCRIPTOR,
				     USB_DESCRIPTOR_HUB, 0, &desc, sizeof(desc));
	if (err) {
	...
	hub->udev = udev;
	err = usb_hub_init(hub, desc.bNbrPorts);
	if (err)
		return err;
