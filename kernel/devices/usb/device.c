...
{
	if (address == USB_DEFAULT_ADDRESS || address > USB_MAX_ADDRESS || address <= 0)
		return;

	spinlock_acquire(&usb_addresses_lock);
	bitmap_clear(usb_addresses, address - 1);
	spinlock_release(&usb_addresses_lock);
}
 ...
 */
error_t usb_device_request_raw(struct usb_device *udev, u8 bRequestType, u8 bRequest, u16 wValue,
...
error_t usb_device_request(struct usb_device *udev, u8 bRequest, u16 wValue, u16 wIndex, void *data,
			   u16 data_size)
{
	return usb_device_request_raw(udev, USB_SETUP_REQ_TYPE_FOR_DEVICE, bRequest, wValue, wIndex,
				      data, data_size);
}

error_t usb_endpoint_request(struct usb_device *udev, u8 bRequest, u16 wValue, u16 wIndex,
			     void *data, u16 data_size)
{
	return usb_device_request_raw(udev, USB_SETUP_REQ_TYPE_FOR_EP, bRequest, wValue, wIndex,
				      data, data_size);
}

error_t usb_interface_request(struct usb_device *udev, u8 bRequest, u16 wValue, u16 wIndex,
			      void *data, u16 data_size)
{
	return usb_device_request_raw(udev, USB_SETUP_REQ_TYPE_FOR_IFACE, bRequest, wValue, wIndex,
				      data, data_size);
}
 ...
 *
 */
static error_t __usb_device_get_descriptor(struct usb_device *udev, u8 recipient, u8 type, u8 index,
					   u16 lang_id, void *data, u16 data_size)
{
	...
	return usb_device_request_raw(udev, recipient, USB_REQUEST_GET_DESCRIPTOR,
				      ((u16)type << 8) | index, lang_id, data, data_size);
}
 ...
 */
error_t
usb_device_get_descriptor(struct usb_device *udev, u8 type, u8 index, void *data, u16 data_size)
{
	return __usb_device_get_descriptor(udev, USB_SETUP_REQ_TYPE_FOR_DEVICE, type, index, 0,
					   data, data_size);
}
 ...
 */
error_t
usb_interface_get_descriptor(struct usb_device *udev, u8 type, u8 index, void *data, u16 data_size)
{
	return __usb_device_get_descriptor(udev, USB_SETUP_REQ_TYPE_FOR_IFACE, type, index, 0, data,
					   data_size);
}
 ...
 *
 */
error_t usb_device_get_string_descriptor(struct usb_device *udev, u8 index, u16 lang_id, void *data,
					 u16 data_size)
{
	return __usb_device_get_descriptor(udev, USB_SETUP_REQ_TYPE_FOR_DEVICE,
					   USB_DESCRIPTOR_STRING, index, lang_id, data, data_size);
}

/*
 *
 */
error_t usb_interface_get_string_descriptor(struct usb_device *udev, u8 index, u16 lang_id,
					    void *data, u16 data_size)
{
	return __usb_device_get_descriptor(udev, USB_SETUP_REQ_TYPE_FOR_IFACE,
					   USB_DESCRIPTOR_STRING, index, lang_id, data, data_size);
}
...
static int __usb_driver_match(const void *node, const void *data)
{
	...
	struct usb_driver *driver = container_of(node, struct usb_driver, driver.this);
...
usb_device_find_driver(u16 id_vendor, u16 id_product, u8 class, u8 subclass, u8 protocol)
{
...
static error_t usb_driver_probe(struct usb_driver *driver, struct usb_device *udev)
{
	device_set_name(&udev->dev, "%s@%u.%u", driver->driver.name, udev->controller->bus,
			udev->address);
...
error_t usb_device_probe(struct usb_device *udev)
{
	...
	struct usb_configuration_descriptor *cfg = (void *)cfg_full;
	error_t err;

	err = usb_device_get_descriptor(udev, USB_DESCRIPTOR_DEVICE, 0, &desc, sizeof(desc));
	if (err) {
	...
	log_info("%s device connected (idVendor=%04x, idProduct=%04x, address=%d)",
		 usb_speed_names[udev->speed], desc.idVendor, desc.idProduct, udev->address);

	err = usb_device_get_descriptor(udev, USB_DESCRIPTOR_CONFIGURATION, 0, &cfg_full,
					sizeof(cfg_full));
	if (err) {
	...
	err = usb_device_request(udev, USB_REQUEST_SET_CONFIGURATION, cfg->bConfigurationValue, 0,
				 NULL, 0);
	if (err) {
	...
	driver = usb_device_find_driver(desc.idVendor, desc.idProduct, desc.bDeviceClass,
