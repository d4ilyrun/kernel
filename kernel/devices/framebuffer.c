...
static const u32 fb_color_palette[COLOR_COUNT] = {
    [COLOR_NONE] = 0x00000000,	       [COLOR_BLACK] = 0x00000000,
    [COLOR_BLUE] = 0xFF0000CC,	       [COLOR_GREEN] = 0xFF00CC00,
    [COLOR_CYAN] = 0xFF00CCCC,	       [COLOR_RED] = 0xFFCC0000,
    [COLOR_MAGENTA] = 0xFFCC00CC,      [COLOR_YELLOW] = 0xFFCCCC00,
    [COLOR_WHITE] = 0xFFCCCCCC,

    [COLOR_BOLD_RED] = 0xFFFF5555,     [COLOR_BOLD_GREEN] = 0xFF55FF55,
    [COLOR_BOLD_YELLOW] = 0xFFFFFF55,  [COLOR_BOLD_BLUE] = 0xFF5555FF,
    [COLOR_BOLD_MAGENTA] = 0xFFFF55FF, [COLOR_BOLD_CYAN] = 0xFF55FFFF,
 ...
 *
 */
static inline bool fb_valid_pixel(const struct framebuffer *fb, unsigned int i, unsigned int j)
{
 ...
 *
 */
static inline off_t fb_pixel_offset(const struct framebuffer *fb, unsigned int i, unsigned int j)
{
 ...
 *
 */
static void fb_put_pixel(const struct framebuffer *fb, unsigned int i, unsigned int j, u32 rgba)
{
	off_t pixel_offset;
 ...
 */
static void
fb_put_char(const struct framebuffer *fb, unsigned int i, unsigned int j, unsigned char c)
{
 ...
 */
static ssize_t
framebuffer_console_write(const struct console *console, const char *buffer, size_t bytes)
{
...
static void framebuffer_console_set_color(const struct console *console,
					  enum console_color fg_color, enum console_color bg_color)
{
	struct framebuffer *fb;
...
static struct device_driver framebuffer_driver = {.name = "framebuffer"};

/*
 *
 */
error_t framebuffer_register(paddr_t buffer, const struct framebuffer_params *params)
{
	struct framebuffer *fb;
	struct device *dev;
	size_t bufsize;
	...
	snprintk(fb->name, sizeof(fb->name), "fb%d", atomic_inc(&framebuffer_count));
