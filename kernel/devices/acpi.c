...
error_t acpi_init(struct multiboot_info *mbt)
{
	...
	if (uacpi_unlikely_error(ret)) {
		log_err("Failed to load AML namespace: %s", uacpi_status_to_string(ret));
		return E_INVAL;
	}

	ret = uacpi_namespace_initialize();
	if (uacpi_unlikely_error(ret)) {
		log_err("Failed to initialize AML namespace: %s", uacpi_status_to_string(ret));
		return E_INVAL;
	}

	ret = uacpi_finalize_gpe_initialization();
	if (uacpi_unlikely_error(ret)) {
		log_err("Failed to finalize GPE initialization: %s", uacpi_status_to_string(ret));
		return E_INVAL;
	}
...
static uacpi_ns_iteration_decision acpi_start_one_device(void *ctx, uacpi_namespace_node *node)
{
	UNUSED(ctx);
...
{
	uacpi_namespace_for_each_node_depth_first(uacpi_namespace_root(), acpi_start_one_device,
						  UACPI_NULL);
	return E_SUCCESS;
}
