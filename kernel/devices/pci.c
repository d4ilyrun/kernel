	...
	while (true) {
		...
		if (compatible->id.device && compatible->id.vendor) {
			...
			if (compatible->class.class == pci_dev->class.class &&
				...
				/* Some device types do not use the interface value. */
				return compatible->class.interface == PCI_CLASS_IFACE_EMPTY ||
				       compatible->class.interface == pci_dev->class.interface;
		} else {
			break;
		}
	}
...
static inline struct pci_device_id pci_read_header_id(uint8_t bus, uint8_t device)
{
	u32 id = pci_read_header(bus, device, ID);
	return *(struct pci_device_id *)&id;
}

static inline struct pci_device_class pci_read_header_class(uint8_t bus, uint8_t device)
{
	uint32_t class = pci_read_header(bus, device, CLASS_CODE);
	return *(struct pci_device_class *)&class;
}
...
static struct pci_bus *pci_bridge_register(struct pci_device *device)
{
	...
	log_dbg("registered bridge %d [parent: %d, subordinate: %d]", bridge->number,
		parent->number, subordinate->number);
...
static void pci_device_setup_bars(struct pci_device *device)
{
	...
	for (int i = 0; i < PCI_BAR_MAX_COUNT; ++i) {
		bar = pci_device_read_config(device, PCI_HEADER_BAR_OFFSET(i), PCI_HEADER_BAR_SIZE);
		if (!bar)
			continue;

		device->bars[i].type = bar & PCI_BAR_MEMORY_SPACE_MASK;

		pci_device_write_config(device, PCI_HEADER_BAR_OFFSET(i), PCI_HEADER_BAR_SIZE,
					ALL_ONES);
		...
		case PCI_BAR_MEMORY:
			...
			device->bars[i].data = vm_alloc_at(&kernel_address_space,
							   device->bars[i].phys,
		...
		pci_device_write_config(device, PCI_HEADER_BAR_OFFSET(i), PCI_HEADER_BAR_SIZE, bar);
	}
}
