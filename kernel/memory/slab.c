...
static void kmem_slab_destroy(struct kmem_slab *slab)
{
    ...
    while ((bufctl = next) != NULL) {
        next = bufctl->next;
        kmem_cache_free(&kmem_bufctl_cache, next);
    }
...
kmem_slab_free_obj(struct kmem_slab *slab, struct kmem_bufctl *bufctl)
{
    ...
    /*
     * NOTE: The slab lists are protected by the interrupts being disabled,
     *       but this design does not work in a multiprocessor environment.
     */
 ...
 *
 * This is the large object version of @ref kmem_slab_init_objects().
 */
...
void kmem_cache_free(struct kmem_cache *cache, void *obj)
{
    ...
    /*
     * Pages allocated by the slab allocator always have a backing physical page
     * since we access them to create the freelist in kmem_cache_grow().
     */
    paddr = mmu_find_physical((vaddr_t)obj);
    if (WARN_ON_MSG(paddr == (paddr_t)-E_INVAL, "freeing invalid object")) {
        return;
    }

    page = address_to_page(paddr);
    if (WARN_ON(!(page->flags & PAGE_SLAB))) {
        return;
    }

    /*
     * FIXME: Does not work ! In the current design, all objects inside a page
     * point to the same bufctl structure. Must use an hashmap (addr -> bufctl)
     * as described in the original paper.
     */

    slab = page->slab.bufctl->slab;
    WARN_ON(slab->cache != cache);

    kmem_cache_lock(cache);
    kmem_slab_free_obj(slab, page->slab.bufctl);
    kmem_cache_unlock(cache);
}
...
int kmem_cache_api_init(void)
{
    ...
    kmem_cache_init(&kmem_slab_cache, "kmem_slab", sizeof(struct kmem_slab), 1,
                    kmem_cache_empty_constructor, kmem_cache_empty_destructor);

    kmem_cache_constructor(&kmem_bufctl_cache);
    kmem_cache_init(&kmem_bufctl_cache, "kmem_bufctl",
                    sizeof(struct kmem_bufctl), 1, kmem_cache_empty_constructor,
                    kmem_cache_empty_destructor);
