...
static void kmem_slab_destroy(struct kmem_slab *slab)
{
	...
	if (atomic_read(&slab->refcount)) {
		log_warn("%s: slab@%p has %d active entries when destroying", cache->name,
			 slab->page, atomic_read(&slab->refcount));
	}
 ...
 *
 */
static void kmem_slab_free_obj(struct kmem_slab *slab, struct kmem_bufctl *bufctl)
{
...
void kmem_cache_free(struct kmem_cache *cache, void *obj)
{
	...
	if (WARN_ON_MSG(IS_ERR(paddr), "free: no backing page for object at %p", obj)) {
		return;
	}

	page = address_to_page(paddr);
	if (WARN_ON_MSG(!(page->flags & PAGE_SLAB),
			"free: object at %p is not a slab object (flags: %04x)", obj,
			page->flags)) {
		return;
	}
 ...
 */
static void kmem_cache_init(struct kmem_cache *cache, const char *name, size_t obj_size,
			    int obj_align, void (*constructor)(void *), void (*destructor)(void *))
{
	cache->name = name;
	...
	if (obj_size >= KMEM_SLAB_LARGE_SIZE) {
		...
		cache->obj_real_size = align_up(obj_size + sizeof(struct kmem_bufctl), obj_align);
	}
}
...
int kmem_cache_api_init(void)
{
	...
	kmem_cache_init(&kmem_cache_cache, "kmem_cache", sizeof(struct kmem_cache), 1,
			kmem_cache_constructor, kmem_cache_destructor);

	kmem_cache_constructor(&kmem_slab_cache);
	kmem_cache_init(&kmem_slab_cache, "kmem_slab", sizeof(struct kmem_slab), 1, NULL, NULL);

	kmem_cache_constructor(&kmem_bufctl_cache);
	kmem_cache_init(&kmem_bufctl_cache, "kmem_bufctl", sizeof(struct kmem_bufctl), 1, NULL,
			NULL);
