...
static inline bool pmm_is_last_pageframe(pmm_frame_allocator *allocator, paddr_t pageframe)
{
...
static bool pmm_initialize_pages(struct multiboot_info *mbt)
{
	...
	for (entry = mmap->entries; (void *)entry < multiboot_tag_end(mmap);
		...
		if (entry->type == MULTIBOOT_MEMORY_AVAILABLE) {
			for (paddr_t addr = entry->addr; addr < entry->addr + entry->len;
				...
				// its code.
				if (IN_RANGE(KERNEL_HIGHER_HALF_VIRTUAL(addr), KERNEL_CODE_START,
					     KERNEL_CODE_END))
					continue;
	...
	log_dbg("First available pageframe: " FMT32, g_pmm_allocator.first_available);

	return true;
}

static void pmm_allocator_allocate_at(pmm_frame_allocator *allocator, paddr_t address, size_t size)
{
	...
	for (address += size; !pmm_is_last_pageframe(allocator, address); address += PAGE_SIZE) {
...
static void pmm_allocator_free_at(pmm_frame_allocator *allocator, paddr_t pageframe)
{
	if (allocator->first_available == PMM_INVALID_PAGEFRAME ||
...
void pmm_free_pages(paddr_t pageframe, size_t size)
{
	if (RANGES_OVERLAP(KERNEL_HIGHER_HALF_VIRTUAL(pageframe),
			   KERNEL_HIGHER_HALF_VIRTUAL(pageframe) + size, KERNEL_CODE_END,
			   KERNEL_CODE_START)) {
		log_err("Trying to free kernel code pages: [" FMT32 "-" FMT32 "]", pageframe,
			pageframe + (native_t)size);
		return;
	}
