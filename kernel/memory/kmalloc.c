...
void kfree(void *ptr)
{
    ...
    bufctl = page->slab.bufctl;
    kmem_cache_free(bufctl->slab->cache, ptr);
}
...
void *krealloc(void *ptr, size_t size, int flags)
{
    ...
    if (page->slab.bufctl->slab->cache->obj_size >= size)
        return ptr;
...
void kmalloc_api_init(void)
{
    ...
    for (int i = 0; i < KMALLOC_CACHE_COUNT; ++i, obj_size <<= 1) {
        cache = kmem_cache_create(kmalloc_cache_names[i], obj_size, 16,
                                  kmem_cache_empty_constructor,
